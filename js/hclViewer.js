var HCLViewer = function() {
    ...
    function load(cdtUrl, atrUrl, gtrUrl)
    {
        cdtFileUrl = cdtUrl;
        atrFileUrl = atrUrl;
        gtrFileUrl = gtrUrl;

        if(gpLib.isGenomeSpaceFile(cdtUrl))
        {
            _retrieveFinalGSUrl(cdtUrl);
        }
        else
        {
            _loadUrl(cdtUrl);
    ...
    function _loadUrl(fileUrl, options)
    {
        ...
        var xhrFields = {};
        if(options !== undefined && options.xhrFields !== undefined)
        {
        ...
        var heatMap = new gpVisual.HeatMap(
        {
            ...
            onLoadData: function(status)
            {
                ...
                {
                    if(atrFileUrl !== undefined)
                    {
                        heatMap.loadAtrFile(atrFileUrl);
                    }

                    if(gtrFileUrl !== undefined)
                    {
                        heatMap.loadGtrFile(gtrFileUrl);
                    }
                }
            }
        });
        ...
        $("#fileLoaded").empty();
        $("#fileLoaded").append("<span>Loaded: <a href='" + fileUrl + "'>" + decodeURIComponent(datasetFileName) + "</a></span>");
...
$(function()
{
    ...
    var cdtFileUrl = requestParams["cdt.file"];
    var gtrFileUrl = requestParams["gtr.file"];
    var atrFileUrl = requestParams["atr.file"];

    if(cdtFileUrl !== undefined && cdtFileUrl.length > 0)
    {
        cdtFileUrl = cdtFileUrl[0];
        //Work around for the GenePattern protocols

        if(cdtFileUrl == "ftp://ftp.broadinstitute.org/pub/genepattern/datasets/protocols/all_aml_test.preprocessed.cdt") {
          cdtFileUrl = "https://www.broadinstitute.org/cancer/software/genepattern/data/protocols/all_aml_test.preprocessed.cdt";
        }
    }
    else
    {
        $("#hclViewerMain").append("<p class='error'> Error: A cdt file must be specified.</p>");
        return;
    }

    if(gtrFileUrl !== undefined && gtrFileUrl.length > 0)
    {
        gtrFileUrl = gtrFileUrl[0];
        //Work around for the GenePattern protocols

        if(gtrFileUrl == "ftp://ftp.broadinstitute.org/pub/genepattern/datasets/protocols/all_aml_test.preprocessed.gtr")
        {
            gtrFileUrl = "https://www.broadinstitute.org/cancer/software/genepattern/data/protocols/all_aml_test.preprocessed.gtr";
        }
    }

    if(atrFileUrl !== undefined && atrFileUrl.length > 0)
    {
        atrFileUrl = atrFileUrl[0];
        if(atrFileUrl[0] == "ftp://ftp.broadinstitute.org/pub/genepattern/datasets/protocols/all_aml_test.preprocessed.atr")
        {
            atrFileUrl = "https://www.broadinstitute.org/cancer/software/genepattern/data/protocols/all_aml_test.preprocessed.atr";
        }
    }

    if((gtrFileUrl == undefined || gtrFileUrl.length == 0) && (atrFileUrl == undefined || atrFileUrl.length == 0))
    {
        $("#hclViewerMain").append("<p class='error'> Error: Either an atr or gtr file must be specified.</p>");
        return;
    }

    HCLViewer.load(cdtFileUrl, atrFileUrl, gtrFileUrl);
